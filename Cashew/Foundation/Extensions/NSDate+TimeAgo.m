...
{
    //DDLogDebug(@"%@", [NSBundle bundleWithPath:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"NSDateTimeAgo.bundle"]]);
...
+ (NSDateFormatter *)_dateFormatterWithoutYear
{
    static dispatch_once_t onceToken;
    static NSDateFormatter *formatter;
    dispatch_once(&onceToken, ^{
...
- (NSString *)timeAgoSimpleForWeekOrLessAndUserShortForm:(BOOL)shortForm;
{
    //DDLogDebug(@"%@", [NSBundle bundleWithPath:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"NSDateTimeAgo.bundle"]]);
    NSDate *now = [NSDate date];
    double deltaSeconds = fabs([self timeIntervalSinceDate:now]);
    ...
    if (shortForm) {
        return [[[self class] _shortDateFormatter] stringFromDate:self];
    }
    
    if ([self _isCurrentYear]) {
        return [[[self class] _dateFormatterWithoutYear] stringFromDate:self];
    } else {
        return [[[self class] _dateFormatter] stringFromDate:self];
    }
}

- (NSString *)timeAgoForWeekOrLessAndDidUseFullDate:(BOOL *)didUseFullDate;
{
    NSDate *now = [NSDate date];
    double deltaSeconds = fabs([self timeIntervalSinceDate:now]);
    double deltaMinutes = deltaSeconds / 60.0f;
...
- (BOOL)_isCurrentYear
{
    ...
    NSDateComponents *todayComponents = [[NSCalendar currentCalendar] components:NSCalendarUnitYear fromDate:[NSDate new]];
